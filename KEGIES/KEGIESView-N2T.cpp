	...
	/*,flannMatcher(cv::NORM_L2)*/
{
	...
	cv::Mat tempIntrinsic;
	tempIntrinsic = cv::Mat(3,3,CV_32F,intrinsic);
	tempIntrinsic.copyTo(m_CamIntrinsic);	
	myText.Format("\nCammatrix = %f\n",m_CamIntrinsic.at<float>(0,0));	
...
void CKEGIESView::UpdateView()
{
	...
	glLoadIdentity();
	if (m_CamMode == 2)  // Look from third person view
	{
	...
	{		
		...
		/*GLfloat matrixmodelview[16] = {
			0.0160,    0.9045,    0.4261,         0,
			-0.9564,   -0.1104,    0.2703,         0,
			0.2915,  -0.4119,    0.8633,         0,
			5.7271,    0.8531,  -12.9613,    1.0000
		};
		glLoadMatrixf(matrixmodelview);*/

		// p3P method result
		/*GLfloat matrixmodelview[16] = {    
			0.0134 ,   0.9020,    0.4315   ,      0,
			-0.9579,   -0.1122,    0.2643 ,        0,
			0.2868,   -0.4169,  0.8625 ,        0,
			5.7320,    0.8596,  -12.9422,    1.0000,
		};
		glLoadMatrixf(matrixmodelview);*/
		...
		/*GLfloat matrixmodelview[16] = { 0.394033 ,    -0.632229,   -0.667101,         0,
		0.918649,    0.248264,   0.307326 ,         0,
		-0.028683 ,    -0.733928,    0.678621 ,         0,
		-6.028140,    -2.418180 ,  -13.464488 ,    1.0000};
		glLoadMatrixf(matrixmodelview);*/
...
void CKEGIESView::OnTimer(UINT_PTR nIDEvent)
{
	...
	case ONLINE_TRACKING:
		...
		*/
		...
		cv::imwrite("output/matches.jpg",img_matches);
		cv::imwrite("output/currentframe1.jpg",m_CurrentFrame);*/
		...
		myText.Format("Cammatrix = %f", m_CamIntrinsic.at<float>(0,0));
		::OutputDebugStringA(myText);
		//solvePnP(objPoints_selected,keyPoints_selected, m_CamIntrinsic, distCoeffs, rvec, tvec, false, cv::EPNP);
		solvePnPRansac(objPoints_selected,keyPoints_selected, 
						m_CamIntrinsic, distCoeffs, rvec, tvec, 
						true, 100, 8.0f,100,inliers,cv::EPNP);		
...
LRESULT CKEGIESView::OnCalculateKeypoints( WPARAM wParam, LPARAM lParam )
{
	...
	cv::imshow("Keypoints Extraction",tempImg);
	CString siftText;
